...
public class CameraCapturerCompat implements VideoCapturer {
    ...
    public CameraCapturerCompat(Context context, Source cameraSource) {
        if (Camera2Capturer.isSupported(context)) {
            Log.e("CAMERA_TYPE", "Camera2Capturer");
            ...
            Log.e("CAMERA_TYPE", "Camera1Capturer");
            setCamera1Maps();
    ...
    public void setZoom(float zoom) {
        if (camera2Capturer != null) {
            camera2Capturer.updateCaptureRequest(captureRequestBuilder -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    // SCALER_CROP_REGION
                    captureRequestBuilder.set(CaptureRequest.CONTROL_ZOOM_RATIO, zoom);
                    //  captureRequestBuilder.set(CaptureRequest.SCALER_CROP_REGION, CaptureRequest.CONTROL_AF_MODE_MACRO);
    ...
    public void startCapture(int width, int height, int framerate) {
        Log.e("DIMES", "startCapture: " + width + " " + height + " " + framerate);
//        val orientationField: Field = VideoFrame::class.java.getDeclaredField("orientation")
//        orientationField.setAccessible(true)
//        orientationField.set(videoFrame, VideoFrame.RotationAngle.ROTATION_180)
    ...
    private void setCamera2Maps(Context context) {
        ...
        for (String cameraId : camera2Enumerator.getDeviceNames()) {
            if (isCameraIdSupported(cameraId)) {
                if (camera2Enumerator.isFrontFacing(cameraId) && !gotFrontCamera) {
                    camera2IdMap.put(Source.FRONT_CAMERA, cameraId);
                    camera2SourceMap.put(cameraId, Source.FRONT_CAMERA);
                    gotFrontCamera = true;
                }
                if (camera2Enumerator.isBackFacing(cameraId) && !gotBackCamera) {
